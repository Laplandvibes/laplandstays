 ...
 * and feeds it to its <SEO> wrapper. We parse that META object straight from
 * SOURCE TEXT (no evaluation) and emit one entry per locale, so the shared
 * ../_prerender_routes.mjs picks it up via its --meta reader (tried FIRST) and
 * bakes localized meta into the prerendered HTML for every locale — instead of the
 * English routes.json fallback placeholder ("LaplandStays — Privacy.").
 *
 ...
 * (routes.json copyFile/pageFile/fallback). Strings are kept verbatim (JA/ZH/KO
 * full-width punctuation, FR apostrophes).
 *
...
// "cookies d'affiliation"); single quotes otherwise.
const STR = `'((?:\\\\.|[^'\\\\])*)'`;
const STR_DQ = `"((?:\\\\.|[^"\\\\])*)"`;
