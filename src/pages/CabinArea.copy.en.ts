import type { CabinAreaPageCopy } from './CabinArea.copy.types'

const copy: CabinAreaPageCopy = {
  eyebrow: 'From the Lomarengas listings',
  areas: {
    levi: {
      seo: {
        title: 'Levi cabins: log cabins and chalets to rent | LaplandStays',
        description: 'Levi cabins and holiday homes on one page: log cabins, ski-in apartments and villas in Sirkka and Kittilä with guests, bedrooms and size for every cabin. Booking on Lomarengas.',
      },
      h1: 'Cabins in Levi',
      intro: 'Lomarengas lists {count} rental cabins around Levi, from Sirkka village to Kittilä: log cabins on the fell slopes, ski-in apartments next to the lifts and large villas for a whole group. Every cabin below shows guests, bedrooms, size and star rating; the weekly price and free weeks are on Lomarengas.',
    },
    yllas: {
      seo: {
        title: 'Ylläs cabins: Äkäslompolo and Ylläsjärvi | LaplandStays',
        description: 'Ylläs cabins and holiday homes: log cabins, lakeside cabins and ski-in apartments in Äkäslompolo and Ylläsjärvi with guests, bedrooms and size for every cabin. Booking on Lomarengas.',
      },
      h1: 'Cabins in Ylläs',
      intro: 'Lomarengas lists {count} rental cabins around Ylläs and its two villages: Äkäslompolo on the north side of the fell and Ylläsjärvi by the southern slopes. Log cabins on forest slopes, lakeside cabins and ski-in apartments. Every cabin below shows guests, bedrooms, size and star rating; the weekly price and free weeks are on Lomarengas.',
    },
    ruka: {
      seo: {
        title: 'Ruka cabins: holiday homes around Rukatunturi | LaplandStays',
        description: 'Ruka cabins and holiday homes: the slope village and the Kuusamo lake country with guests, bedrooms and size for every cabin on one page. Booking on Lomarengas.',
      },
      h1: 'Cabins in Ruka',
      intro: 'Lomarengas lists {count} rental cabins around Ruka, from the slope village to the lakeshores of Kuusamo. Ruka opens its slopes among the first in autumn and the season runs into spring, so cabin weeks are available for a long stretch. Every cabin below shows guests, bedrooms, size and star rating; the weekly price and free weeks are on Lomarengas.',
    },
    saariselka: {
      seo: {
        title: 'Saariselkä cabins: holiday homes below Kaunispää | LaplandStays',
        description: 'Saariselkä cabins and holiday homes: log cabins and apartments around the village and towards Kakslauttanen with guests, bedrooms and size for every cabin. Booking on Lomarengas.',
      },
      h1: 'Cabins in Saariselkä',
      intro: 'Lomarengas lists {count} rental cabins around Saariselkä and Kaunispää, on the edge of Urho Kekkonen National Park. It is the northernmost of the four ski resorts, so the aurora nights are darkest here. Every cabin below shows guests, bedrooms, size and star rating; the weekly price and free weeks are on Lomarengas.',
    },
    'pyha-luosto': {
      seo: {
        title: 'Pyhä and Luosto cabins: holiday homes to rent | LaplandStays',
        description: 'Pyhätunturi and Luosto cabins and holiday homes in Pelkosenniemi, Sodankylä and Kemijärvi with guests, bedrooms and size for every cabin. Booking on Lomarengas.',
      },
      h1: 'Cabins in Pyhä and Luosto',
      intro: 'Lomarengas lists {count} rental cabins between the two fells of Pyhä and Luosto, next to Pyhä-Luosto National Park. A quieter alternative to Levi and Ruka, and under two hours from Rovaniemi airport. Every cabin below shows guests, bedrooms, size and star rating; the weekly price and free weeks are on Lomarengas.',
    },
    rovaniemi: {
      seo: {
        title: 'Rovaniemi cabins: holiday homes on the Arctic Circle | LaplandStays',
        description: 'Rovaniemi cabins and holiday homes: riverside cabins and wilderness huts along the Ounasjoki and Kemijoki with guests, bedrooms and size for every cabin. Booking on Lomarengas.',
      },
      h1: 'Cabins in Rovaniemi',
      intro: 'Lomarengas lists {count} rental cabins along the rivers and in the villages around Rovaniemi. You can drive to the cabin straight from the airport, and Santa Claus Village and the city services are a short drive away. Every cabin below shows guests, bedrooms, size and star rating; the weekly price and free weeks are on Lomarengas.',
    },
  },
  guestsLabel: 'guests',
  bedroomsLabel: 'bedrooms',
  sizeLabel: 'size',
  starsLabel: 'stars',
  viewCabin: 'View cabin',
  weekFrom: 'week from {price} €',
  priceOnPartner: 'Price and free weeks on Lomarengas',
  browseAll: 'Browse every cabin in the area on Lomarengas',
  showMore: 'Show more cabins',
  dataNote: 'Photos and details from the Lomarengas product feed; weekly prices refresh daily for the cabins that show one. Booking happens on the Lomarengas site.',
  otherAreasH2: 'Cabins in the other resorts',
  destinationLink: 'Hotels, igloos and the guide for this destination',
  breadcrumbHome: 'Home',
  breadcrumbCabins: 'Cabin holidays',
}

export default copy
