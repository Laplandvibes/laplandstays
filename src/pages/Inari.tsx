...
For extended wilderness trips, the road continues to Utsjoki and the Norwegian border.`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: 'On the quiet eastern shore of Lake Inari, far from village lights. Fishing, saunas and aurora trips out on the lake ice.' },
  ...
  transport: [
    { mode: 'plane', label: 'From Ivalo Airport (IVL)', detail: 'Direct flights from Helsinki year-round; seasonal winter routes from e.g. London Gatwick and Paris. 50 km / 40 min north to Inari village.' },
    { mode: 'bus', label: 'Airport transfer to Inari', detail: 'Pre-booked charter or taxi from IVL (~40 min). The walk-on airport bus only runs towards Saariselkä.' },
...
Pidemmille erämaamatkoille tie jatkuu Utsjoelle ja Norjan rajalle.`,
  ...
  transport: [
    { mode: 'plane', label: 'Ivalon lentokentältä (IVL)', detail: 'Suorat lennot Helsingistä ympäri vuoden; talvikaudella kausireittejä mm. Lontoosta (Gatwick) ja Pariisista. 50 km / 40 min pohjoiseen Inarin kylälle.' },
    { mode: 'bus', label: 'Kenttäkuljetus Inariin', detail: 'Esivarattava tilausajo tai taksi IVL:ltä (~40 min). Ilman varausta kenttäbussi kulkee vain Saariselän suuntaan.' },
...
Für ausgedehnte Wildnisreisen führt die Straße weiter nach Utsjoki und zur norwegischen Grenze.`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: 'Am stillen Ostufer des Inarisees, fern der Dorflichter. Angeln, Saunen und Polarlicht-Touren auf dem Seeeis.' },
  ...
  transport: [
    { mode: 'plane', label: 'Ab Flughafen Ivalo (IVL)', detail: 'Direktflüge ab Helsinki ganzjährig; im Winter Saisonrouten u. a. ab London-Gatwick und Paris. 50 km / 40 min nordwärts ins Dorf Inari.' },
    { mode: 'bus', label: 'Flughafentransfer nach Inari', detail: 'Vorab gebuchter Transfer oder Taxi ab IVL (~40 min). Der Flughafenbus ohne Reservierung fährt nur Richtung Saariselkä.' },
...
이곳은 깊은 북쪽입니다. 캐빈들은 진정한 의미로 외져 있습니다. 마을에서 차로 10분만 달려도 180° 지평선 전체에 다른 불빛이라곤 보이지 않는 호반에 닿을 수 있습니다. 이나리 마을의 SIIDA 박물관이 사미의 유산을 단단히 지키며, 주변 야생은 Utsjoki 국경의 툰드라 고원까지 이어집니다.`,
  ...
  highlights: [
    { title: '이나리 호수의 독점성', body: '호반의 숙소는 의도적으로 멀찍이 떨어져 있습니다. 전용 부두, 겨울철 얼음 낚시 구멍, 여름에는 신성한 우코 섬(Ukonkivi) 옆을 지나는 보트 크루즈를 즐길 수 있습니다(섬 상륙은 더 이상 허용되지 않습니다).' },
...
더 긴 야생 여행을 원하시면 도로는 Utsjoki와 노르웨이 국경까지 이어집니다.`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: '이나리 호수 동쪽 기슭, 마을 불빛에서 멀리. 낚시, 사우나, 호수 얼음 위 오로라 투어.' },
  ...
  transport: [
    { mode: 'plane', label: '이발로 공항(IVL)에서', detail: '헬싱키발 직항은 연중 운항, 겨울에는 런던 개트윅·파리 등 시즌 노선. 이나리 마을까지 북쪽으로 50km / 40분.' },
    { mode: 'bus', label: '이나리행 공항 교통', detail: 'IVL에서 사전 예약 셔틀 또는 택시(약 40분). 예약 없이 타는 공항버스는 사리셀카 방면만 운행합니다.' },
  ...
  dayPlan: [
    ...
    { day: '03', title: '이나리 호수 스노모빌 투어', body: '가이드와 함께 스노모빌로 얼어붙은 호수 위를 달립니다. 신성한 우코 섬이 수평선 위로 보입니다(섬에는 오르지 않습니다). 겨울에는 14:30이면 어두워지므로 캐빈으로 돌아와 긴 오로라 관측 시간대를 누리세요.' },
...
Pour une virée plus longue dans la nature, la route continue jusqu\'à Utsjoki et la frontière norvégienne.`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: 'Sur la rive est du lac Inari, loin des lumières des villages. Pêche, saunas et sorties aurores sur le lac gelé.' },
  ...
  transport: [
    { mode: 'plane', label: "Depuis l'aéroport d'Ivalo (IVL)", detail: 'Vols directs depuis Helsinki toute l\'année ; en hiver, liaisons saisonnières depuis Londres-Gatwick et Paris notamment. 50 km / 40 min vers le nord jusqu\'au village d\'Inari.' },
    { mode: 'bus', label: 'Transfert aéroport vers Inari', detail: 'Navette à réserver ou taxi depuis IVL (~40 min). Le bus sans réservation ne dessert que la direction de Saariselkä.' },
...
Per viaggi più lunghi nella natura, la strada prosegue fino a Utsjoki e al confine norvegese.`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: 'Sulla riva orientale del lago Inari, lontano dalle luci dei paesi. Pesca, saune e uscite per l\'aurora sul lago ghiacciato.' },
  ...
  transport: [
    { mode: 'plane', label: 'Dall\'aeroporto di Ivalo (IVL)', detail: 'Voli diretti da Helsinki tutto l\'anno; in inverno rotte stagionali ad es. da Londra Gatwick e Parigi. 50 km / 40 min verso nord fino al paese di Inari.' },
    { mode: 'bus', label: 'Transfer aeroporto–Inari', detail: 'Navetta su prenotazione o taxi da IVL (~40 min). Il bus senza prenotazione serve solo la direzione di Saariselkä.' },
...
Voor langere wildernisritten gaat de weg verder naar Utsjoki en de Noorse grens.`,
  ...
  transport: [
    { mode: 'plane', label: 'Vanaf luchthaven Ivalo (IVL)', detail: 'Directe vluchten vanaf Helsinki het hele jaar; in de winter seizoensroutes vanaf o.a. Londen Gatwick en Parijs. 50 km / 40 min noordwaarts naar het dorp Inari.' },
    { mode: 'bus', label: 'Luchthaventransfer naar Inari', detail: 'Vooraf geboekte shuttle of taxi vanaf IVL (~40 min). De bus zonder reservering rijdt alleen richting Saariselkä.' },
...
ここは深北です。キャビンは文字どおりの僻地に建ち、村から車で10分も走れば、180度の地平線に他の灯りひとつ見えない湖岸に立てます。イナリ村のSIIDA博物館がサーミの伝統を支え、周囲の原野はウツヨキ国境のツンドラ台地へとつながっています。`,
  ...
  highlights: [
    { title: 'イナリ湖の独占感', body: '湖畔の宿は意図的に離して配置されています。プライベート桟橋、冬はアイスフィッシング、夏は聖なるウコンキヴィ島のそばを巡るボートクルーズが楽しめます（島への上陸は現在認められていません）。' },
...
原野へのロングトリップでは、道路はウツヨキとノルウェー国境まで続きます。`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: 'イナリ湖の静かな東岸、集落の明かりから離れて。釣り、サウナ、氷上のオーロラツアー。' },
...
Para viajes prolongados al territorio salvaje, la carretera continúa a Utsjoki y a la frontera con Noruega.`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: 'En la tranquila orilla oriental del lago Inari, lejos de las luces de los pueblos. Pesca, saunas y salidas de auroras sobre el hielo del lago.' },
  ...
  transport: [
    { mode: 'plane', label: 'Desde el aeropuerto de Ivalo (IVL)', detail: 'Vuelos directos desde Helsinki todo el año; en invierno rutas estacionales desde Londres-Gatwick y París, entre otras. 50 km / 40 min al norte hasta el pueblo de Inari.' },
    { mode: 'bus', label: 'Traslado del aeropuerto a Inari', detail: 'Traslado reservado o taxi desde IVL (~40 min). El bus sin reserva solo va hacia Saariselkä.' },
...
Para viagens prolongadas na natureza, a estrada continua até Utsjoki e a fronteira com a Noruega.`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: 'Na tranquila margem leste do Lago Inari, longe das luzes dos vilarejos. Pesca, saunas e passeios de aurora sobre o gelo do lago.' },
  ...
  transport: [
    { mode: 'plane', label: 'Do Aeroporto de Ivalo (IVL)', detail: 'Voos diretos de Helsinque o ano todo; no inverno, rotas sazonais de Londres-Gatwick e Paris, entre outras. 50 km / 40 min ao norte até a vila de Inari.' },
    { mode: 'bus', label: 'Transfer do aeroporto até Inari', detail: 'Transfer reservado ou táxi do IVL (~40 min). O ônibus sem reserva vai apenas na direção de Saariselkä.' },
...
const zhCN: DestinationBody = {
  tagline: '薩米文化首府、芬蘭第三大湖，以及芬蘭拉普蘭最偏遠的小屋。',
  description: `伊納里是芬蘭薩米文化的首府，也是芬蘭面積最大的市鎮。伊納里湖(Inarijärvi)面積1,040平方公里，點綴著3,000座小島，是芬蘭第三大湖，也是歐洲最清澈的水體之一。
...
這裡是深北。這裡的小屋是真正意義上的偏遠：從村莊驅車十分鐘，就可置身於180°地平線上看不到其他燈光的湖岸。伊納里村中的SIIDA博物館是薩米文化的核心，周圍的荒野延伸至烏茨約基邊境的苔原高原。`,
  ...
  highlights: [
    { title: '伊納里湖的獨享感', body: '湖岸住宿刻意彼此疏遠。私人碼頭、冬季冰釣孔、夏季可乘船巡遊，途經神聖的Ukonkivi島（現已禁止登島）。' },
...
8月中旬迎來短暫而濃烈的ruska（秋色）。苔原一片紅橙。`,
  howToGet: `飛往伊瓦洛(IVL)，接送40分鐘。每日有赫爾辛基。伊瓦洛直飛，冬季還有來自英國與中歐的季節包機。
租車。最棒的小屋位於湖畔小路，需要私人車輛。
若進行長途荒野旅行，公路繼續通往Utsjoki及挪威邊境。`,
  stayTypes: [
    '伊納里湖畔原木小屋。可住4–6人，私人湖岸、柴燒桑拿、冰釣裝備。',
    '偏遠荒野別墅。脫網般的體驗與完整舒適，任何方向都是幾小時的未受干擾森林。',
    '薩米家庭經營的小屋。小巧而具個性，常伴有馴鹿放牧傳統與現場講述。',
    '伊納里村以北的極光玻璃靜修屋。專為高緯度觀星而設計。',
  ],
  siblings: [
    { name: 'Saariselkä', href: '/destinations/saariselka', blurb: '通往UKK荒野的入口，南行50分鐘。' },
    { name: 'Rovaniemi', href: '/destinations/rovaniemi', blurb: '拉普蘭首府，主要的抵達地點。' },
    { name: 'Levi', href: '/destinations/levi', blurb: '大型滑雪村莊，可步行的中心區。' },
  ],
  anchorProperties: [
  ...
  carRental: {
    ...
    sid: 'destination_inari_cars_ivl',
    airport: '伊瓦洛機場(IVL)取車',
    blurb: '至伊納里村40分鐘 · Nellim與伊納里湖畔小屋必備',
  },
  dayPlan: [
...
För längre vildmarksresor fortsätter vägen till Utsjoki och norska gränsen.`,
  ...
  anchorProperties: [
    { name: 'Wilderness Hotel Nellim', propertyQuery: 'Wilderness Hotel Nellim', sid: 'destination_inari_nellim', note: 'Vid Inarisjöns stilla östra strand, långt från byarnas ljus. Fiske, bastur och norrskensturer ute på sjöisen.' },
  ...
  transport: [
    { mode: 'plane', label: 'Från Ivalo flygplats (IVL)', detail: 'Direktflyg från Helsingfors året runt; vintertid säsongsrutter från bl.a. London Gatwick och Paris. 50 km / 40 min norrut till Inari by.' },
    { mode: 'bus', label: 'Flygplatstransfer till Inari', detail: 'Förbokad transfer eller taxi från IVL (~40 min). Bussen utan bokning går bara mot Saariselkä.' },
