...
const COPY: Record<Lang, CookieCopy> = {
  ...
  'zh-CN': {
    ...
    whatAreTitle: '什麼是 Cookie?',
    whatAreBody: 'Cookie 是您訪問網站時儲存在您裝置上的小型文本檔案。它們幫助網站記住您的偏好，並瞭解您如何使用該網站。您可以通過瀏覽器設定或通過我們的同意橫幅來管理 Cookie。',
    cookiesWeUseTitle: '我們使用的 Cookie',
    essentialBadge: '必要',
    essentialNote: '始終啟用，不可關閉',
    essentialBody: '這些 Cookie 是網站正常執行所必需的。它們儲存您的同意偏好，這樣我們就無需在每次訪問時再次詢問。',
    ...
    gygBody: (siteName) => `當您在本網站使用 GetYourGuide 小元件或點選活動連結時，GetYourGuide（德國柏林，歐盟）可能會設定 Cookie，以追蹤小元件的互動並將後續預訂歸因於 ${siteName}（合作伙伴 ID VRMKD7N）。預訂本身在 getyourguide.com 上完成，適用其自身的隱私政策。`,
    gygDur: '保留期限：會話至30天',
    lsBadge: 'localStorage',
    lsNote: '儲存在您的瀏覽器中，絕不會發送到伺服器',
    lsIntro: '為了減少打擾，我們在您瀏覽器的 localStorage 中儲存了兩個小條目：',
    lsConsentDesc: '您的接受/拒絕選擇（上述同意 Cookie 的映象）',
    lsPopupDesc: '您上次關閉或成功訂閱電子報彈窗的時間戳，以便在7天內不再顯示（若已訂閱則永不顯示）',
    lsTail: '嚴格來說 localStorage 並不是 Cookie。我們在此列出僅為了透明。您可以通過瀏覽器的網站資料設定將其清除。',
    tableTitle: 'Cookie 一覽表',
    tableCookie: 'Cookie',
    tableType: '類型',
    tablePurpose: '用途',
    tableDuration: '保留期限',
    ...
    managing1: '您可以隨時通過在瀏覽器中清除本網站的 Cookie 來更改您的同意，這樣下次訪問時同意橫幅會再次出現。您也可以在瀏覽器設定中完全停用 Cookie，但這可能會影響網站功能。',
    managing2: '大多數瀏覽器都允許檢視、管理和刪除 Cookie。請參閱您瀏覽器的幫助部分獲取具體說明。',
    contactTitle: '聯絡方式',
    contactBody: (email) => <>如對 Cookie 或本政策有任何問題，請通過 {email} 與我們聯絡。</>,
    backToHome: '← 返回首頁',
    privacyLink: '隱私政策 →',
    typeEssential: '必要',
...
export default function CookieContent({
  ...
  const purposeStrings: Record<Lang, { consent: string; popup: string; gaUser: string; gaSession: string; cj: string; gyg: string }> = {
    ...
    'zh-CN': {
      consent: '儲存您的 Cookie 同意偏好（已接受/已拒絕）',
      popup: '記住您是否關閉或訂閱了電子報彈窗，以避免重複顯示',
      gaUser: 'Google Analytics，區分獨立使用者',
      gaSession: 'Google Analytics，維護會話狀態',
      cj: 'Adtraction、Travelpayouts 與 Trip.com，當您點選聯盟預訂連結時歸因佣金。經由 do.sembo.fi / on.lomarengas.fi / tp.media 設定，或作為 trip.com 連結引數',
      gyg: 'GetYourGuide 小元件：追蹤小元件互動並將預訂歸因於本網站',
    },
    ko: {
  ...
  const durStrings: Record<Lang, { oneYear: string; cjDur: string; gygDur: string; popupDur: string; gaDur: string }> = {
    ...
    'zh-CN': { oneYear: '1年', cjDur: '不等（會話至45天）', gygDur: '會話至30天', popupDur: '7天（關閉後）/ 永久（訂閱後）', gaDur: '14個月' },
