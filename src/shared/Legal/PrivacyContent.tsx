...
const COPY: Record<Lang, {
  ...
  'zh-CN': {
    ...
    s1Title: '1. 資料控制者',
    s1Body: () => <>LaPeso Oy（企業程式碼 3309136-7），芬蘭。電子郵件：<a href="mailto:info@laplandvibes.com" className="text-vibe-pink">info@laplandvibes.com</a></>,
    s2Title: '2. 我們收集的資料',
    s2Body: '我們通過 Google Analytics 4 收集假名化的訪問分析資料。如果您訂閱了我們的電子報，我們會安全地儲存您的電子郵件地址。除非您直接與我們聯絡，否則我們不會收集任何其他可識別個人身份的資訊。',
    s2aTitle: '2a. 處理的法律依據（通用資料保護條例第6條）',
    s2aIntro: '我們針對每一項處理活動依據以下法律依據進行處理：',
    s2aItems: [
      { strong: '同意(第6(1)(a)條)', body: '用於分析 cookie(Google Analytics 4)及其他非必要 cookie。您通過 cookie 橫幅給予同意，並可隨時撤回。' },
      { strong: '同意(第6(1)(a)條)', body: '用於電子報訂閱。您通過提交訂閱表單給予同意，可隨時通過取消訂閱連結撤回。' },
      { strong: '合法利益(第6(1)(f)條)', body: '用於必要 cookie（儲存您的同意偏好）以及防欺詐和安全日誌。我們的利益是運營一個可正常使用的網站，並已與您的合理期望相平衡。' },
      { strong: '合法利益(第6(1)(f)條)', body: '用於聯盟連結點選歸因。我們的利益是獲得編輯工作所應得的佣金；收集的資料極少（來源），您也可以選擇不點選聯盟連結。' },
    ],
    s3Title: '3. Cookie',
    s3Intro: '我們的網站使用 cookie 以提升您的瀏覽體驗並收集假名化分析資料。這些包括：',
    s3Items: [
      { strong: '必要 cookie', body: '網站正常執行所必需（同意偏好、會話資料）。' },
      { strong: '分析 cookie', body: 'Google Analytics 4 用於瞭解訪客如何與本網站互動。以假名化方式收集。' },
      { strong: '聯盟 cookie', body: '當您點選聯盟連結（例如 Adtraction 或 Daisycon 或 Travelpayouts 追蹤）時設定，用於歸因推薦佣金。' },
    ],
    s3Tail: (cookieLink) => <>分析 cookie 僅在您通過 cookie 橫幅同意後才會設定。詳情請參閱我們的{cookieLink}。</>,
    s4Title: '4. Google Analytics',
    s4Body: '我們使用啟用了 Consent Mode v2 的 Google Analytics 4。如果您拒絕 Cookie，則不會收集任何分析資料。如果您同意，使用資料（瀏覽的頁面、停留時間、裝置類型，以及國家和城市級別的位置）會發送給 Google。這些資料是假名化的：我們不會發送您的姓名、電子郵件等可直接識別身份的資訊，但 Cookie 中的隨機識別符號和您的 IP 地址屬於 GDPR 所稱的個人資料。',
    s5Title: '5. 電子報',
    s5Body: (unsub) => <>如果您訂閱了我們的電子報，您的電子郵件地址將通過 Resend 和 Supabase 安全儲存。您可以隨時通過每封郵件中的連結或通過我們的{unsub}取消訂閱。</>,
    s6Title: '6. 資料保留',
    s6Body: '分析資料在 Google Analytics 中保留14個月。電子報的電子郵件地址在您取消訂閱前一直保留。',
    s7Title: '7. 第三方',
    s7Intro: '我們不會向第三方出售或共享您的個人資料。但作為運營的一部分，以下第三方服務會處理資料：',
    s7Items: [
      'Google Analytics，假名化使用分析',
      'Adtraction、Daisycon、Travelpayouts 與 Trip.com，點選預訂與合作伙伴連結的追蹤',
      'Resend，電子報郵件傳送',
      'Supabase，後端資料庫服務',
      'Cloudflare，託管與 CDN',
    ],
    s8Title: '8. 廣告',
    s8Body1: (siteName) => `本網站會展示第三方廣告主的贊助內容。贊助內容會清晰標註"贊助"標識。點選贊助連結可能將您重定向到擁有自身隱私政策的外部網站。${siteName} 不對外部廣告主的資料處理做法負責。`,
    s8Body2: '我們通過 Adtraction 和 Daisycon 和 Travelpayouts 網路以及 Trip.com 合作伙伴計劃參與聯盟計劃，合作伙伴包括 Sembo、Lomarengas、Trip.com 和 EconomyBookings。當您點選聯盟連結並完成購買或預訂時，我們可能獲得佣金，而您無需承擔任何額外費用。',
    s8aTitle: '8a. 跨境資料傳輸',
    s8aIntro: '我們使用的若干第三方服務的總部或資料傳輸目的地位於歐洲經濟區(EEA)以外，最常見的是美國：',
    s8aItems: [
      { strong: 'Google Analytics', body: '（Google LLC，美國）：受 EU–US 資料隱私框架(DPF)保護。' },
      { strong: 'Cloudflare', body: '（Cloudflare Inc.，美國）：受 EU–US 資料隱私框架及標準合同條款(SCC)保護。' },
      { strong: 'Resend', body: '（Resend Inc.，美國）：受標準合同條款(SCC)保護。' },
      { strong: 'Supabase', body: '（Supabase Inc.，美國，亦可使用歐盟區域託管）：受標準合同條款(SCC)保護。' },
      { strong: 'Adtraction / Daisycon / Travelpayouts / Trip.com', body: '（Adtraction AB，瑞典，歐盟；Daisycon B.V.，荷蘭，歐盟；Travelpayouts 與 Trip.com，國際）：歐盟/歐洲經濟區以外的傳輸受標準合同條款(SCC)保護。' },
      { strong: 'GetYourGuide', body: '（GetYourGuide GmbH，德國）：位於歐洲經濟區內。' },
    ],
    s8aTail: '在任何情形下，資料傳輸均通過歐盟委員會的充分性決定、EU–US 資料隱私框架或經歐盟委員會批准的標準合同條款進行保護。您可聯絡我們索取相應保護措施的副本。',
    s9Title: '9. 您在通用資料保護條例下的權利',
    s9Intro: '由於我們在芬蘭運營並向歐盟訪客提供服務，通用資料保護條例(GDPR)完全適用。您享有以下權利：',
    s9Items: [
      { strong: '訪問權（第15條）', body: '請求獲取我們持有的關於您個人資料的副本。' },
      { strong: '更正權（第16條）', body: '要求我們更正不準確或不完整的資料。' },
      { strong: '刪除權 / "被遺忘權"（第17條）', body: '在不存在壓倒性保留理由時，要求我們刪除您的資料。' },
      { strong: '限制處理權（第18條）', body: '在問題正在解決期間，要求我們暫停處理。' },
      { strong: '資料可攜帶權（第20條）', body: '以結構化、機器可讀的格式獲取您的資料。' },
      { strong: '反對權（第21條）', body: '反對基於合法利益的處理，包括直接營銷。' },
      { strong: '撤回同意權', body: '可隨時撤回，自撤回時刻起生效。' },
      { strong: '投訴權（第77條）', body: '向芬蘭資料保護專員公署(Tietosuojavaltuutettu,tietosuoja.fi)或您在歐盟慣常居住地的監管機構投訴。' },
    ],
    s9Tail: (email) => <>如需行使上述任何權利，請通過 {email} 與我們聯絡。我們將在一個月內回覆。</>,
    s10Title: '10. 自動化決策',
    s10Body: '我們不會進行通用資料保護條例第22條所述的、對您產生法律或類似重大影響的自動化決策、使用者畫像或其他相關處理。',
    s11Title: '11. 未成年人',
    s11Body: '本網站及我們的電子報面向成年人。我們不會有意收集13歲以下兒童的資料（芬蘭法律及通用資料保護條例規定的數位服務年齡門檻）。如果您認為某位兒童向我們提供了個人資料，請聯絡我們，我們將予以刪除。',
    s12Title: '12. 本政策的變更',
    s12Body: '我們可能會不時更新本隱私政策。頂部的"最後更新"日期反映最新修訂。重大變更將在主頁上至少醒目展示14天。',
    backToHome: '← 返回首頁',
