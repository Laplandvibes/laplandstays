...
const COPY = {
  ...
  'zh-CN': {
    eyebrow: '您為何而來',
    h2: '旅行者記住的3件事',
    lead: '無論您計劃什麼。哈士奇、滑雪、馴鹿、精緻餐飲。這些是客人回到家裡首先要說起的事。',
    items: [
      { tag: '寂靜', headline: '沒有嗡鳴。沒有車流。沒有鄰居。', body: '入夜後走上露臺，世界彷彿靜止。腳下積雪咯吱作響，遠離路燈的夜空綴滿星星。寂靜完整得讓耳朵嗡嗡作響。第一晚幾乎會被它嚇到；到第三晚，您已捨不得放手。' },
      { tag: '溫暖', headline: '每個晚上，都有桑拿。', body: '從嚴寒中回到屋裡，爐子已經燒得通紅。蒸汽升騰，窗外雪光閃爍，一天的寒意從肩頭融化。在這裡，私人桑拿是標配而非奢華加價項。到第二個晚上，您會發現自己開始圍繞它安排一整天。' },
      { tag: '天空', headline: '極光叫醒與玻璃天花板。', body: '深夜，提醒輕輕響起。您把被子拉到下巴，看著綠光在床頂的玻璃上流轉。從9月到4月，北極天空就是住宿的一部分。選對房間，您甚至不必走進寒夜就能看到它。' },
    ],
  },
  de: {
